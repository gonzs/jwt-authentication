...
program
  .version("0.0.1")
  .description(
    "Tool to make new service and its respective controller and an entry in routes"
  );
...
const createServiceFolder = foldername => {
  return new Promise((resolve, reject) => {
    const folderPath = process.cwd() + "/app/services/" + foldername;
...
const createServiceFile = filename => {
  createServiceFolder(filename)
    .then((folderPath, error) => {
...
const createControllerFolder = controllerFolder => {
  return new Promise((resolve, reject) => {
    const folderPath =
      process.cwd() + "/app/controllers/apis/" + controllerFolder;
...
const createControllerFile = controllerFile => {
  createControllerFolder(controllerFile)
    .then((folderPath, error) => {
      const filePath = folderPath + "/index.js";
      shell.touch(filePath);
      ...
      content +=
        "router.post('/', " + controllerFile + "Service.AddYourMethod);\n\n";
...
const addEntryToRoutes = (apiVersion, controllerName) => {
  let apiRoutePath = "./app/routes/apis/" + apiVersion + "/index.js";
...
const readApiRouteFile = (controllerName, apiVersion) => {
  let lr = new lineByLineReader(
    "./app/routes/apis/" + apiVersion + "/index.js"
  );
  let lines = [];
  let row = 0;
      ...
      if (line.length == 0) {
        if (row === 1) {
          lines.push(
            "const " +
              controllerName +
              "Controller = require('../../../controllers/apis/" +
              controllerName +
              "');"
          );
        }
